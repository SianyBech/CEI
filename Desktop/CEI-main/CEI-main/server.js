...
async function authenticateRequest(req, res) {
  ...
  if (isDemoSession(accessToken, refreshToken)) {
    return { user: getDemoUser(), session: { access_token: 'demo-access-token', refresh_token: 'demo-refresh-token' } };
  }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (isDemoCredentials(email, password)) {
      const demoSession = { access_token: 'demo-access-token', refresh_token: 'demo-refresh-token' };
      setSessionCookies(res, demoSession);
      return res.json({ user: getDemoUser(), session: demoSession });
    }
