window.CerneApp.Header = {
  render(onNewEvidenceClick, onSettingsClick, onLogout, user = null) {
    const currentUser = user || window.CerneApp.Auth?.currentUserProfile || window.CerneApp.Auth?.getCurrentUser?.() || null;
    ...
    const userColor = user?.cor || currentUser?.cor || currentUser?.user_metadata?.cor || '#0066cc';
